...
const PAYLOAD_URL = process.env.NEXT_PUBLIC_PAYLOAD_URL || 'https://payload.ra0.cn'
const PAYLOAD_API_KEY = process.env.PAYLOAD_API_KEY || ''

// 媒体库返回的 url 可能已是绝对地址（https://payload.ra0.cn/api/media/file/...），
// 此时不能再拼 PAYLOAD_URL，否则会变成 https://…https://… 双 URL 导致缩略图裂图。
function toAbsUrl(url: string): string {
  return /^https?:\/\//.test(url) ? url : `${PAYLOAD_URL}${url}`
}

// 当前站点标识
const SITE_SLUG = 'lov0u'
...
export async function getAllArticleSlugs(): Promise<string[]> {
  try { const { articles } = await getArticles(1, 100); return articles.map((a) => a.slug) }
  catch { return [] }
}
...
export async function getTags(): Promise<Tag[]> {
  try {
    ...
    if (!res.ok) { console.error('Payload API error:', res.status); return [] }
    const data = await res.json()
    return (data.docs || []).map((item: any) => ({
      id: String(item.id), name: item.name || item.slug, slug: item.slug || `tag-${item.id}`,
    }))
  } catch (error) { console.error('Failed to fetch tags:', error); return [] }
}

export async function getArticlesByTag(
  tagSlug: string, page = 1, pageSize = 20
  ...
  try {
    ...
    const tagRes = await fetch(tagUrl, { headers: getHeaders(), next: { revalidate: 3600 } })
    let tag: Tag | null = null
    if (tagRes.ok) {
      const tagData = await tagRes.json()
      if (tagData.docs && tagData.docs.length > 0) {
        tag = { id: String(tagData.docs[0].id), name: tagData.docs[0].name || tagData.docs[0].slug, slug: tagData.docs[0].slug }
      }
    }
    ...
    const res = await fetch(url, { headers: getHeaders(), next: { revalidate: 3600 } })
    if (!res.ok) { console.error('Payload API error:', res.status); return { articles: [], total: 0, tag } }
    const data = await res.json()
    const articles: ArticleListItem[] = (data.docs || []).map(mapArticleListItem)
    return { articles, total: data.total || 0, tag }
  } catch (error) { console.error('Failed to fetch articles by tag:', error); return { articles: [], total: 0, tag: null } }
}

export async function getAllTagSlugs(): Promise<string[]> {
  try { const tags = await getTags(); return tags.map((t) => t.slug) } catch { return [] }
}
